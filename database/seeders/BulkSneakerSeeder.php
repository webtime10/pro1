...
/**
 * 10 000 кроссовок (RU + EN), опции и атрибуты, фото catalog/110.png.
 *
 * php artisan db:seed --class=BulkSneakerSeeder
 * php artisan scout:import "App\Models\Product"
 * php artisan catalog:unique-product-images
 */
class BulkSneakerSeeder extends Seeder
{
    use EnsuresCatalogAttributes, EnsuresCatalogManufacturers;

    private const IMAGE_PATH = 'catalog/110.png';

    private const TOTAL = 10000;

    private const CHUNK = 500;

    private const SKU_PREFIX = 'BULK-SNEAKER-';
    ...
    public function run(): void
    {
        ...
        Product::withoutSyncingToSearch(function () use ($ru, $en) {
            ...
            $stylesRu = ['Беговые', 'Городские', 'Тренировочные', 'Lifestyle', 'Баскетбольные', 'Треккинговые', 'Слипоны', 'High-top', 'Low-top', 'Premium'];
            $stylesEn = ['Running', 'Urban', 'Training', 'Lifestyle', 'Basketball', 'Trail', 'Slip-on', 'High-top', 'Low-top', 'Premium'];

            $materialsRu = ['Сетка + синтетика', 'Кожа', 'Нубук', 'Текстиль', 'Замша', 'Комбинированные'];
            $materialsEn = ['Mesh + synthetic', 'Leather', 'Nubuck', 'Textile', 'Suede', 'Mixed materials'];

            $solesRu = ['Пена React', 'EVA', 'Резина', 'Air cushion', 'Двойная подошва'];
            $solesEn = ['React foam', 'EVA', 'Rubber', 'Air cushion', 'Dual sole'];

            $closuresRu = ['Шнурки', 'Липучка', 'Slip-on', 'Быстрая шнуровка'];
            $closuresEn = ['Laces', 'Velcro', 'Slip-on', 'Quick lace'];

            $seasonsRu = ['Всесезонные', 'Лето', 'Демисезон', 'Зима'];
            $seasonsEn = ['All-season', 'Summer', 'Mid-season', 'Winter'];
            ...
            for ($start = 1; $start <= self::TOTAL; $start += self::CHUNK) {
                ...
                DB::transaction(function () use (
                    ...
                    $solesRu, $solesEn, $closuresRu, $closuresEn, $seasonsRu, $seasonsEn
                ) {
                    for ($i = $start; $i <= $end; $i++) {
                        ...
                        $nameRu = $stylesRu[$styleIdx].' кроссовки #'.$i;
                        $nameEn = $stylesEn[$styleIdx].' sneakers #'.$i;
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameRu,
                            'slug' => 'krossovki-'.$num,
                            'description' => '<p>Демо-кроссовки №'.$i.' ('.$stylesRu[$styleIdx].').</p>',
                            'tag' => 'кроссовки, демо, bulk',
                            'meta_title' => $nameRu,
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameEn,
                            'slug' => 'sneakers-'.$num,
                            'description' => '<p>Demo sneakers #'.$i.' ('.$stylesEn[$styleIdx].').</p>',
                            'tag' => 'sneakers, demo, bulk',
                            'meta_title' => $nameEn,
                        ...
                        if ($i % 5 === 0) {
                            $this->attachWidthOption($product, $options['width'], $options['width_values'], $i);
                        }
                    }
                });
        ...
        $this->command?->info('Готово: '.self::TOTAL.' кроссовок (RU + EN), SKU '.self::SKU_PREFIX.'00001 … '.self::SKU_PREFIX.'10000');
    ...
    {
        $public = public_path('catalog/110.png');
        ...
        if (! is_file($storage) && ! is_file($public)) {
            $this->command?->error('Нет фото '.self::IMAGE_PATH.' — положите файл в storage/app/public/catalog/110.png');
        }
    }
    ...
    private function ensureCategory(Language $ru, Language $en): Category
    {
        ...
        CategoryDescription::create([
            ...
            'name' => 'Кроссовки (bulk)',
            'slug' => 'krossovki-bulk',
            'description' => '10000 демо-кроссовок',
        ]);
        ...
        CategoryDescription::create([
            ...
            'name' => 'Sneakers (bulk)',
            'slug' => 'sneakers-bulk',
            'description' => '10000 demo sneakers',
        ]);
    ...
    private function ensureAttributes(Language $ru, Language $en): array
    {
        $group = AttributeGroup::query()
            ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', 'Характеристики кроссовок'))
            ->first();

        if (! $group) {
            $group = AttributeGroup::create(['sort_order' => 10]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $ru->id,
                'name' => 'Характеристики кроссовок',
            ]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $en->id,
                'name' => 'Sneaker specifications',
            ]);
        }

        $defs = [
            'material' => ['Материал верха', 'Upper material'],
            'sole' => ['Подошва', 'Sole'],
    ...
    {
        $size = $this->ensureOption($ru, $en, 'select', 'Размер обуви', 'Shoe size', 20);
        $color = $this->ensureOption($ru, $en, 'radio', 'Цвет кроссовок', 'Sneaker color', 21);
        $width = $this->ensureOption($ru, $en, 'select', 'Полнота', 'Width', 22);
        ...
        return [
            ...
            'color_values' => $this->ensureOptionValues(
                $color, $ru, $en,
                ['Чёрный', 'Белый', 'Серый', 'Синий', 'Красный', 'Зелёный'],
                ['Black', 'White', 'Gray', 'Blue', 'Red', 'Green']
            ),
            'width' => $width,
            'width_values' => $this->ensureOptionValues(
                $width, $ru, $en,
                ['Стандарт', 'Узкая', 'Широкая'],
    ...
    /** @param  array<int, OptionValue>  $values */
    private function attachWidthOption(Product $product, Option $option, array $values, int $index): void
    {
        $count = 2 + ($index % 2);
        $subset = array_slice($values, 0, min($count, count($values)));
