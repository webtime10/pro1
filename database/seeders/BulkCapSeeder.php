...
/**
 * 10 000 кепок (RU + EN), опции и атрибуты, фото catalog/105.png.
 *
 * php artisan db:seed --class=BulkCapSeeder
 * php artisan scout:import "App\Models\Product"
 */
class BulkCapSeeder extends Seeder
{
    use EnsuresCatalogAttributes, EnsuresCatalogManufacturers;

    private const IMAGE_PATH = 'catalog/105.png';

    private const TOTAL = 10000;

    private const CHUNK = 500;

    private const SKU_PREFIX = 'BULK-CAP-';
    ...
    public function run(): void
    {
        ...
        Product::withoutSyncingToSearch(function () use ($ru, $en) {
            ...
            $stylesRu = ['Бейсболка', 'Тракер', 'Snapback', 'Dad cap', 'Плоская', 'Спортивная', 'Уличная', 'Premium', 'Vintage', 'Classic'];
            $stylesEn = ['Baseball', 'Trucker', 'Snapback', 'Dad cap', 'Flat brim', 'Sport', 'Street', 'Premium', 'Vintage', 'Classic'];

            $materialsRu = ['100% хлопок', 'Хлопок с полиэстером', 'Сетка + хлопок', 'Полиэстер', 'Суэта', 'Деним'];
            $materialsEn = ['100% cotton', 'Cotton with polyester', 'Mesh + cotton', 'Polyester', 'Suede', 'Denim'];

            $visorsRu = ['Изогнутый козырёк', 'Прямой козырёк', 'Короткий козырёк'];
            $visorsEn = ['Curved visor', 'Flat visor', 'Short visor'];

            $closuresRu = ['Липучка', 'Snapback', 'Фиксированный размер', 'Резинка сзади'];
            $closuresEn = ['Velcro', 'Snapback', 'Fixed size', 'Elastic back'];

            $printsRu = ['Без принта', 'Вышивка', 'Нашивка', 'Полная печать', 'Минимальный логотип'];
            $printsEn = ['Plain', 'Embroidery', 'Patch', 'Full print', 'Minimal logo'];
            ...
            for ($start = 1; $start <= self::TOTAL; $start += self::CHUNK) {
                ...
                DB::transaction(function () use (
                    ...
                    $visorsRu, $visorsEn, $closuresRu, $closuresEn, $printsRu, $printsEn
                ) {
                    for ($i = $start; $i <= $end; $i++) {
                        ...
                        $nameRu = $stylesRu[$styleIdx].' кепка #'.$i;
                        $nameEn = $stylesEn[$styleIdx].' cap #'.$i;
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameRu,
                            'slug' => 'kepka-'.$num,
                            'description' => '<p>Демо-кепка №'.$i.' ('.$stylesRu[$styleIdx].').</p>',
                            'tag' => 'кепка, демо, bulk',
                            'meta_title' => $nameRu,
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameEn,
                            'slug' => 'cap-'.$num,
                            'description' => '<p>Demo cap #'.$i.' ('.$stylesEn[$styleIdx].').</p>',
                            'tag' => 'cap, demo, bulk',
                            'meta_title' => $nameEn,
                        ...
                        $attrData = [
                            ...
                            'visor' => [$visorsRu[$i % count($visorsRu)], $visorsEn[$i % count($visorsEn)]],
                            'closure' => [$closuresRu[$i % count($closuresRu)], $closuresEn[$i % count($closuresEn)]],
                        ...
                        if ($i % 4 === 0) {
                            $this->attachEmbroideryOption($product, $options['embroidery'], $options['embroidery_values'], $i);
                        }
                    }
                });
        ...
        $this->command?->info('Готово: '.self::TOTAL.' кепок (RU + EN), SKU '.self::SKU_PREFIX.'00001 … '.self::SKU_PREFIX.'10000');
    ...
    {
        $public = public_path('catalog/105.png');
    ...
    private function ensureAttributes(Language $ru, Language $en): array
    {
        $group = AttributeGroup::query()
            ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', 'Характеристики кепки'))
            ->first();

        if (! $group) {
            $group = AttributeGroup::create(['sort_order' => 5]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $ru->id,
                'name' => 'Характеристики кепки',
            ]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $en->id,
                'name' => 'Cap specifications',
            ]);
        }
        ...
        $defs = [
            ...
            'visor' => ['Козырёк', 'Visor'],
            'closure' => ['Застёжка', 'Closure'],
            'print' => ['Декор', 'Decoration'],
        ];
    ...
    {
        $size = $this->ensureOption($ru, $en, 'select', 'Размер кепки', 'Cap size', 9);
        $color = $this->ensureOption($ru, $en, 'radio', 'Цвет кепки', 'Cap color', 10);
        $embroidery = $this->ensureOption($ru, $en, 'select', 'Вышивка', 'Embroidery', 11);

        return [
            'size' => $size,
            'size_values' => $this->ensureOptionValues(
                $size, $ru, $en,
                ['One Size', '54 см', '56 см', '58 см', '60 см', '62 см'],
                ['One Size', '54 cm', '56 cm', '58 cm', '60 cm', '62 cm']
            ),
            'color' => $color,
            'color_values' => $this->ensureOptionValues(
                $color, $ru, $en,
                ['Чёрный', 'Белый', 'Синий', 'Красный', 'Хаки', 'Серый'],
                ['Black', 'White', 'Blue', 'Red', 'Khaki', 'Gray']
            ),
            'embroidery' => $embroidery,
            'embroidery_values' => $this->ensureOptionValues(
                $embroidery, $ru, $en,
                ['Без вышивки', 'Логотип спереди', 'Логотип сбоку', 'Полная вышивка'],
                ['No embroidery', 'Front logo', 'Side logo', 'Full embroidery']
            ),
        ];
    }
    ...
    /** @param  array<int, OptionValue>  $values */
    private function attachEmbroideryOption(Product $product, Option $option, array $values, int $index): void
    {
        $count = 2 + ($index % 3);
        $subset = array_slice($values, 0, min($count, count($values)));
