<?php

namespace Database\Seeders;

use App\Models\Attribute;
use App\Models\AttributeDescription;
use App\Models\AttributeGroup;
use App\Models\AttributeGroupDescription;
use App\Models\Category;
use App\Models\CategoryDescription;
use App\Models\Language;
use App\Models\Option;
use App\Models\OptionDescription;
use App\Models\OptionValue;
use App\Models\OptionValueDescription;
use App\Models\Product;
use App\Models\ProductAttribute;
use App\Models\ProductDescription;
use App\Models\ProductOption;
use App\Models\ProductOptionValue;
use Database\Seeders\Concerns\EnsuresCatalogAttributes;
use Database\Seeders\Concerns\EnsuresCatalogManufacturers;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

/**
 * 1000 рубашек (RU + EN), опции и атрибуты, фото catalog/100.webp.
 *
 * php artisan db:seed --class=BulkShirtSeeder
 * php artisan scout:import "App\Models\Product"
 */
class BulkShirtSeeder extends Seeder
{
    use EnsuresCatalogAttributes, EnsuresCatalogManufacturers;

    private const IMAGE_PATH = 'catalog/100.webp';

    private const TOTAL = 1000;

    private const CHUNK = 100;

    private const SKU_PREFIX = 'BULK-SHIRT-';

    public function run(): void
    {
        $ru = Language::query()->where('code', 'ru')->first();
        $en = Language::query()->where('code', 'en')->first();

        if (! $ru || ! $en) {
            $this->command?->error('Нужны языки ru и en. Запустите: php artisan db:seed --class=LanguageSeeder');

            return;
        }

        if (Product::query()->where('sku', 'like', self::SKU_PREFIX.'%')->exists()) {
            $this->command?->warn('Товары '.self::SKU_PREFIX.'* уже есть. Удалите их или смените префикс.');

            return;
        }

        $this->ensureImage();

        Product::withoutSyncingToSearch(function () use ($ru, $en) {
            $category = $this->ensureCategory($ru, $en);
            $attributes = $this->ensureAttributes($ru, $en);
            $options = $this->ensureOptions($ru, $en);

            $stylesRu = ['Классическая', 'Офисная', 'Casual', 'Slim', 'Льняная', 'В клетку', 'В полоску', 'Premium', 'Летняя', 'Business'];
            $stylesEn = ['Classic', 'Office', 'Casual', 'Slim', 'Linen', 'Plaid', 'Striped', 'Premium', 'Summer', 'Business'];

            $materialsRu = ['100% хлопок', 'Хлопок Oxford', 'Хлопок с эластаном', '55% лён, 45% хлопок', 'Хлопок поплин', 'Хлопок twill'];
            $materialsEn = ['100% cotton', 'Oxford cotton', 'Cotton with elastane', '55% linen, 45% cotton', 'Poplin cotton', 'Twill cotton'];

            $sleevesRu = ['Длинный рукав', 'Короткий рукав', '3/4 рукав'];
            $sleevesEn = ['Long sleeve', 'Short sleeve', '3/4 sleeve'];

            $fitsRu = ['Regular Fit', 'Slim Fit', 'Classic Fit', 'Relaxed Fit', 'Tailored Fit'];
            $fitsEn = ['Regular Fit', 'Slim Fit', 'Classic Fit', 'Relaxed Fit', 'Tailored Fit'];

            $careRu = ['Машинная стирка 40°C', 'Деликатная стирка', 'Стирать наизнанку', 'Не требует глажки', 'Химчистка'];
            $careEn = ['Machine wash 40°C', 'Delicate wash', 'Wash inside out', 'No iron needed', 'Dry clean'];

            for ($start = 1; $start <= self::TOTAL; $start += self::CHUNK) {
                $end = min($start + self::CHUNK - 1, self::TOTAL);

                DB::transaction(function () use (
                    $start, $end, $ru, $en, $category, $attributes, $options,
                    $stylesRu, $stylesEn, $materialsRu, $materialsEn,
                    $sleevesRu, $sleevesEn, $fitsRu, $fitsEn, $careRu, $careEn
                ) {
                    for ($i = $start; $i <= $end; $i++) {
                        $num = str_pad((string) $i, 4, '0', STR_PAD_LEFT);
                        $styleIdx = ($i - 1) % count($stylesRu);

                        $product = Product::create([
                            'model' => 'SHIRT-'.$num,
                            'sku' => self::SKU_PREFIX.$num,
                            'quantity' => ($i % 7) + 3,
                            'image' => self::IMAGE_PATH,
                            'manufacturer_id' => $this->catalogManufacturerId($i),
                            'price' => 1500 + ($i % 35) * 100,
                            'sort_order' => $i,
                            'status' => true,
                            'shipping' => true,
                            'subtract' => true,
                            'minimum' => 1,
                        ]);

                        $product->categories()->sync([$category->id]);

                        $nameRu = $stylesRu[$styleIdx].' рубашка #'.$i;
                        $nameEn = $stylesEn[$styleIdx].' shirt #'.$i;

                        ProductDescription::create([
                            'product_id' => $product->id,
                            'language_id' => $ru->id,
                            'name' => $nameRu,
                            'slug' => 'rubashka-'.$num,
                            'description' => '<p>Демо-рубашка №'.$i.' ('.$stylesRu[$styleIdx].').</p>',
                            'tag' => 'рубашка, демо, bulk',
                            'meta_title' => $nameRu,
                            'meta_description' => $nameRu,
                        ]);

                        ProductDescription::create([
                            'product_id' => $product->id,
                            'language_id' => $en->id,
                            'name' => $nameEn,
                            'slug' => 'shirt-'.$num,
                            'description' => '<p>Demo shirt #'.$i.' ('.$stylesEn[$styleIdx].').</p>',
                            'tag' => 'shirt, demo, bulk',
                            'meta_title' => $nameEn,
                            'meta_description' => $nameEn,
                        ]);

                        $attrData = [
                            'material' => [$materialsRu[$i % count($materialsRu)], $materialsEn[$i % count($materialsEn)]],
                            'sleeve' => [$sleevesRu[$i % count($sleevesRu)], $sleevesEn[$i % count($sleevesEn)]],
                            'fit' => [$fitsRu[$i % count($fitsRu)], $fitsEn[$i % count($fitsEn)]],
                            'care' => [$careRu[$i % count($careRu)], $careEn[$i % count($careEn)]],
                        ];

                        foreach ($attrData as $key => [$textRu, $textEn]) {
                            ProductAttribute::create([
                                'product_id' => $product->id,
                                'attribute_id' => $attributes[$key]->id,
                                'language_id' => $ru->id,
                                'text' => $textRu,
                            ]);
                            ProductAttribute::create([
                                'product_id' => $product->id,
                                'attribute_id' => $attributes[$key]->id,
                                'language_id' => $en->id,
                                'text' => $textEn,
                            ]);
                        }

                        $this->attachSizeOption($product, $options['size'], $options['size_values'], $i);
                        $this->attachColorOption($product, $options['color'], $options['color_values'], $i);

                        if ($i % 3 === 0) {
                            $this->attachLengthOption($product, $options['length'], $options['length_values'], $i);
                        }
                    }
                });

                $this->command?->info("Создано товаров: {$end} / ".self::TOTAL);
            }
        });

        $this->command?->info('Готово: '.self::TOTAL.' товаров (RU + EN), SKU '.self::SKU_PREFIX.'0001 … '.self::SKU_PREFIX.'1000');
        $this->command?->line('Фото: /storage/'.self::IMAGE_PATH.' (и public/catalog/100.webp)');
        $this->command?->line('Индекс: php artisan scout:import "App\Models\Product"');
    }

    private function ensureImage(): void
    {
        $public = public_path('catalog/100.webp');
        $storage = storage_path('app/public/'.self::IMAGE_PATH);

        @mkdir(dirname($public), 0755, true);
        @mkdir(dirname($storage), 0755, true);

        if (is_file($public) && ! is_file($storage)) {
            copy($public, $storage);

            return;
        }

        if (is_file($storage) && ! is_file($public)) {
            copy($storage, $public);

            return;
        }

        if (is_file($public) || is_file($storage)) {
            return;
        }

        if (function_exists('imagewebp')) {
            $img = imagecreatetruecolor(600, 600);
            $bg = imagecolorallocate($img, 30, 64, 120);
            $fg = imagecolorallocate($img, 255, 255, 255);
            imagefilledrectangle($img, 0, 0, 600, 600, $bg);
            imagestring($img, 5, 240, 290, '100', $fg);
            imagewebp($img, $public, 80);
            imagewebp($img, $storage, 80);
            imagedestroy($img);
        }
    }

    private function ensureCategory(Language $ru, Language $en): Category
    {
        $existing = CategoryDescription::query()
            ->where('language_id', $ru->id)
            ->where('slug', 'rubashki-bulk')
            ->first();

        if ($existing) {
            $category = Category::findOrFail($existing->category_id);
        } else {
            $category = Category::create([
                'parent_id' => null,
                'image' => self::IMAGE_PATH,
                'top' => true,
                'column' => 1,
                'sort_order' => 1,
                'status' => true,
            ]);

            CategoryDescription::create([
                'category_id' => $category->id,
                'language_id' => $ru->id,
                'name' => 'Рубашки (bulk)',
                'slug' => 'rubashki-bulk',
                'description' => '1000 демо-рубашек',
            ]);

            CategoryDescription::create([
                'category_id' => $category->id,
                'language_id' => $en->id,
                'name' => 'Shirts (bulk)',
                'slug' => 'shirts-bulk',
                'description' => '1000 demo shirts',
            ]);

            Category::rebuildPaths();
        }

        if (! CategoryDescription::query()->where('category_id', $category->id)->where('language_id', $en->id)->exists()) {
            CategoryDescription::create([
                'category_id' => $category->id,
                'language_id' => $en->id,
                'name' => 'Shirts (bulk)',
                'slug' => 'shirts-bulk',
                'description' => '1000 demo shirts',
            ]);
        }

        return $category;
    }

    /** @return array<string, Attribute> */
    private function ensureAttributes(Language $ru, Language $en): array
    {
        $group = AttributeGroup::query()
            ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', 'Характеристики рубашки'))
            ->first();

        if (! $group) {
            $group = AttributeGroup::create(['sort_order' => 1]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $ru->id,
                'name' => 'Характеристики рубашки',
            ]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $en->id,
                'name' => 'Shirt specifications',
            ]);
        }

        $defs = [
            'material' => ['Материал', 'Material'],
            'sleeve' => ['Рукав', 'Sleeve'],
            'fit' => ['Покрой', 'Fit'],
            'care' => ['Уход', 'Care'],
        ];

        $result = [];
        foreach ($defs as $key => [$nameRu, $nameEn]) {
            $result[$key] = $this->firstOrCreateAttributeByName(
                $group,
                $ru,
                $nameRu,
                $en,
                $nameEn,
                count($result) + 1
            );
        }

        return $result;
    }

    /** @return array<string, mixed> */
    private function ensureOptions(Language $ru, Language $en): array
    {
        $size = $this->ensureOption($ru, $en, 'select', 'Размер', 'Size', 1);
        $color = $this->ensureOption($ru, $en, 'radio', 'Цвет', 'Color', 2);
        $length = $this->ensureOption($ru, $en, 'select', 'Длина', 'Length', 3);

        return [
            'size' => $size,
            'size_values' => $this->ensureOptionValues($size, $ru, $en, ['S', 'M', 'L', 'XL', 'XXL']),
            'color' => $color,
            'color_values' => $this->ensureOptionValues(
                $color, $ru, $en,
                ['Белый', 'Голубой', 'Синий', 'Чёрный'],
                ['White', 'Light blue', 'Blue', 'Black']
            ),
            'length' => $length,
            'length_values' => $this->ensureOptionValues(
                $length, $ru, $en,
                ['Обычная', 'Удлинённая'],
                ['Regular', 'Tall']
            ),
        ];
    }

    private function ensureOption(Language $ru, Language $en, string $type, string $nameRu, string $nameEn, int $sort): Option
    {
        $option = Option::query()
            ->where('type', $type)
            ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', $nameRu))
            ->first();

        if ($option) {
            return $option;
        }

        $option = Option::create(['type' => $type, 'sort_order' => $sort]);
        OptionDescription::create(['option_id' => $option->id, 'language_id' => $ru->id, 'name' => $nameRu]);
        OptionDescription::create(['option_id' => $option->id, 'language_id' => $en->id, 'name' => $nameEn]);

        return $option;
    }

    /**
     * @param  array<int, string>  $namesRu
     * @param  array<int, string>|null  $namesEn
     * @return array<int, OptionValue>
     */
    private function ensureOptionValues(Option $option, Language $ru, Language $en, array $namesRu, ?array $namesEn = null): array
    {
        $namesEn = $namesEn ?? $namesRu;
        $values = [];

        foreach ($namesRu as $i => $nameRu) {
            $nameEn = $namesEn[$i] ?? $nameRu;

            $value = OptionValue::query()
                ->where('option_id', $option->id)
                ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', $nameRu))
                ->first();

            if (! $value) {
                $value = OptionValue::create([
                    'option_id' => $option->id,
                    'image' => '',
                    'sort_order' => $i + 1,
                ]);
                OptionValueDescription::create([
                    'option_value_id' => $value->id,
                    'language_id' => $ru->id,
                    'option_id' => $option->id,
                    'name' => $nameRu,
                ]);
                OptionValueDescription::create([
                    'option_value_id' => $value->id,
                    'language_id' => $en->id,
                    'option_id' => $option->id,
                    'name' => $nameEn,
                ]);
            }

            $values[] = $value;
        }

        return $values;
    }

    /** @param  array<int, OptionValue>  $sizeValues */
    private function attachSizeOption(Product $product, Option $option, array $sizeValues, int $index): void
    {
        $count = 3 + ($index % 3);
        $subset = array_slice($sizeValues, 0, $count);

        $productOption = ProductOption::create([
            'product_id' => $product->id,
            'option_id' => $option->id,
            'value' => '',
            'required' => true,
        ]);

        foreach ($subset as $j => $value) {
            ProductOptionValue::create([
                'product_option_id' => $productOption->id,
                'product_id' => $product->id,
                'option_id' => $option->id,
                'option_value_id' => $value->id,
                'quantity' => 5 + (($index + $j) % 10),
                'subtract' => true,
                'price' => 0,
                'price_prefix' => '+',
            ]);
        }
    }

    /** @param  array<int, OptionValue>  $colorValues */
    private function attachColorOption(Product $product, Option $option, array $colorValues, int $index): void
    {
        $count = 2 + ($index % 3);
        $subset = array_slice($colorValues, 0, min($count, count($colorValues)));

        $productOption = ProductOption::create([
            'product_id' => $product->id,
            'option_id' => $option->id,
            'value' => '',
            'required' => $index % 2 === 0,
        ]);

        foreach ($subset as $j => $value) {
            ProductOptionValue::create([
                'product_option_id' => $productOption->id,
                'product_id' => $product->id,
                'option_id' => $option->id,
                'option_value_id' => $value->id,
                'quantity' => 3 + (($index + $j) % 8),
                'subtract' => true,
                'price' => $j * 100,
                'price_prefix' => '+',
            ]);
        }
    }

    /** @param  array<int, OptionValue>  $lengthValues */
    private function attachLengthOption(Product $product, Option $option, array $lengthValues, int $index): void
    {
        $productOption = ProductOption::create([
            'product_id' => $product->id,
            'option_id' => $option->id,
            'value' => '',
            'required' => false,
        ]);

        foreach ($lengthValues as $j => $value) {
            ProductOptionValue::create([
                'product_option_id' => $productOption->id,
                'product_id' => $product->id,
                'option_id' => $option->id,
                'option_value_id' => $value->id,
                'quantity' => 10,
                'subtract' => false,
                'price' => $j === 1 ? 250 : 0,
                'price_prefix' => '+',
            ]);
        }
    }
}
