...
/**
 * 10 000 джинсов (RU + EN), опции и атрибуты, фото catalog/103.webp.
 *
 * php artisan db:seed --class=BulkJeansSeeder
 * php artisan scout:import "App\Models\Product"
 */
class BulkJeansSeeder extends Seeder
{
    use EnsuresCatalogAttributes, EnsuresCatalogManufacturers;

    private const IMAGE_PATH = 'catalog/103.webp';

    private const TOTAL = 10000;

    private const CHUNK = 500;

    private const SKU_PREFIX = 'BULK-JEANS-';
    ...
    public function run(): void
    {
        ...
        Product::withoutSyncingToSearch(function () use ($ru, $en) {
            ...
            $stylesRu = ['Классические', 'Скинни', 'Слим', 'Прямые', 'Мом', 'Бойфренд', 'Трубы', 'Карго', 'Vintage', 'Premium'];
            $stylesEn = ['Classic', 'Skinny', 'Slim', 'Straight', 'Mom', 'Boyfriend', 'Wide leg', 'Cargo', 'Vintage', 'Premium'];

            $materialsRu = ['100% хлопок деним', 'Хлопок с эластаном', 'Смесовый деним', 'Органический хлопок', 'Плотный деним 14 oz', 'Лёгкий деним 10 oz'];
            $materialsEn = ['100% cotton denim', 'Cotton with elastane', 'Blended denim', 'Organic cotton', 'Heavy denim 14 oz', 'Light denim 10 oz'];

            $risesRu = ['Низкая посадка', 'Средняя посадка', 'Высокая посадка'];
            $risesEn = ['Low rise', 'Mid rise', 'High rise'];

            $fitsRu = ['Slim', 'Regular', 'Relaxed', 'Tapered', 'Loose'];
            $fitsEn = ['Slim', 'Regular', 'Relaxed', 'Tapered', 'Loose'];

            $washesRu = ['Тёмно-синий', 'Светло-синий', 'Чёрный', 'Серый', 'Выбеленный', 'Эффект потёртостей'];
            $washesEn = ['Dark blue', 'Light blue', 'Black', 'Gray', 'Bleached', 'Distressed'];
            ...
            for ($start = 1; $start <= self::TOTAL; $start += self::CHUNK) {
                ...
                DB::transaction(function () use (
                    ...
                    $risesRu, $risesEn, $fitsRu, $fitsEn, $washesRu, $washesEn
                ) {
                    for ($i = $start; $i <= $end; $i++) {
                        ...
                        $nameRu = $stylesRu[$styleIdx].' джинсы #'.$i;
                        $nameEn = $stylesEn[$styleIdx].' jeans #'.$i;
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameRu,
                            'slug' => 'dzhinsy-'.$num,
                            'description' => '<p>Демо-джинсы №'.$i.' ('.$stylesRu[$styleIdx].').</p>',
                            'tag' => 'джинсы, демо, bulk',
                            'meta_title' => $nameRu,
                        ...
                        ProductDescription::create([
                            ...
                            'name' => $nameEn,
                            'slug' => 'jeans-'.$num,
                            'description' => '<p>Demo jeans #'.$i.' ('.$stylesEn[$styleIdx].').</p>',
                            'tag' => 'jeans, demo, bulk',
                            'meta_title' => $nameEn,
                        ...
                        $attrData = [
                            ...
                            'wash' => [$washesRu[$i % count($washesRu)], $washesEn[$i % count($washesEn)]],
                        ];
        ...
        $this->command?->info('Готово: '.self::TOTAL.' джинсов (RU + EN), SKU '.self::SKU_PREFIX.'00001 … '.self::SKU_PREFIX.'10000');
    ...
    {
        $public = public_path('catalog/103.webp');
    ...
    private function ensureCategory(Language $ru, Language $en): Category
    {
        ...
        CategoryDescription::create([
            ...
            'name' => 'Джинсы (bulk)',
            'slug' => 'dzhinsy-bulk',
            'description' => '10000 демо-джинсов',
        ]);
    ...
    private function ensureAttributes(Language $ru, Language $en): array
    {
        $group = AttributeGroup::query()
            ->whereHas('descriptions', fn ($q) => $q->where('language_id', $ru->id)->where('name', 'Характеристики джинсов'))
            ->first();

        if (! $group) {
            $group = AttributeGroup::create(['sort_order' => 3]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $ru->id,
                'name' => 'Характеристики джинсов',
            ]);
            AttributeGroupDescription::create([
                'attribute_group_id' => $group->id,
                'language_id' => $en->id,
                'name' => 'Jeans specifications',
            ]);
        }
    ...
    private function ensureOptions(Language $ru, Language $en): array
    {
        ...
        $color = $this->ensureOption($ru, $en, 'radio', 'Оттенок', 'Shade', 7);
        ...
        return [
            ...
            'color_values' => $this->ensureOptionValues(
                $color, $ru, $en,
                ['Индиго', 'Чёрный', 'Серый', 'Светлый синий'],
                ['Indigo', 'Black', 'Gray', 'Light blue']
            ),
        ];
    }
